...
description: Plan scripts and storyboards, analyze music cue points, create character animation and game sprite exports, or edit existing video. Use for GIF, APNG, transparent frames, sprite sheets and video deliverables from text, images, video or music. Grok is the default motion source; other available video tools, image keyframes and local media are alternatives.
metadata:
  version: "1.0.0"
---
...
- **Script/storyboard only:** read [planning](references/planning.md), deliver the written plan and stop without image or video generation.
- **Music analysis only:** read [music](references/music.md), analyze the supplied audio and return candidate cue timestamps with suggested uses and review status. Do not generate or edit footage unless requested.
- **Simple local editing:** read [timeline](references/timeline.md) and [music](references/music.md) when relevant. Prepare source frames before composing trims, reordered segments, retiming, holds, loops and an optional soundtrack. Source clip audio is not automatically retained; clarify an audio-preservation request before using this visual-frame route.
...
For installation requests, follow [references/install.md](references/install.md): have the AI check, download and install missing prerequisites, including Python, rather than return a manual checklist. Preserve working installations; interactive account authorization remains with the user. Use this skill folder's `.venv/bin/python` (Windows: `.venv/Scripts/python.exe`) for its scripts; do not assume the current shell Python has the dependencies. `scripts/doctor.py` reports local readiness without spending generation quota.

**Use Grok by default for new motion and effects unless the user selects another route. Grok is an optional dependency, not a prerequisite for GIF generation.** Other available and authorized generative video tools can replace Grok, or their outputs can be imported. When no suitable video route is available, use an available image tool for appropriate keyframe GIFs; existing media can also be processed locally. Prefer Grok continuous video when available for more natural articulated motion. The Grok route uses **Codex + Grok**: Codex runs this skill, directs the motion, uses available image generation when needed, and processes/verifies the output; Grok supplies generated continuous video. Grok must be authenticated and have usable video-generation entitlement/quota. This version includes its own OAuth REST client, using the official Grok CLI for OAuth sign-in and credential refresh, or an explicitly selected direct API key, as described in [references/grok.md](references/grok.md). Do not imply the repository provides accounts, subscriptions or credits. The installer manages Python packages and missing FFmpeg/ffprobe binaries; read [managed dependencies](references/dependencies.md) when installing or diagnosing tool availability. Existing video/RGBA conversion can run locally without a new Grok call. Grok and fal.ai MiniMax H3 Max clients are bundled. When fal.ai or H3 Max is selected, read [fal connection and modes](references/fal.md) and use `scripts/fal_video.py`; never silently switch models or billing providers. Other providers require available tools or explicit integration. Grok website generation/import needs website login, while the bundled client requires its own CLI OAuth session or explicit API key; website login alone does not authenticate the client.

## Select the route

Generation providers are interchangeable input routes, not the identity of this skill. fal.ai / MiniMax H3 Max is one supported route, never the sole required provider. Apply the same inspection, repair and final-quality gate to Grok, other available authorized generators, user-supplied footage and existing local videos. Preserve the actual provider provenance; a new route requires its own capability/authentication check and does not silently change billing authority.
...
For dirty contours, fragments or flickering edges, read [structure-aware edge repair](references/structural-edge-repair.md). For clean chroma or alpha output, use its material-aware alpha and geometry checks: confirmed backdrop is uniform, opaque surfaces stay opaque, fractional coverage/translucency remains graded, and contours follow perspective and motion without halos or jaggies. Identify the actual component and preserve its material, lighting and occlusion before choosing a local repair; pixel size or blur alone cannot classify a defect. Detached-looking detail may be a clipped real strand: use source-guided recovery in a reviewed mask when connection is missing, rather than automatically deleting it.

For hair popping, crawling fractional edges or hair overlapping clothing, read [fine-hair occlusion and temporal repair](references/hair-occlusion-temporal-repair.md). Verify source continuity, preserve opaque backing surfaces, and test both source-background recomposition and contrasting backgrounds; a successful numeric round trip is not a correct matte. Do not promote an unaccepted local candidate to a library-wide recipe.

For unintended holes beside straps, shoes or narrow limb gaps, use [opaque-material loss diagnosis](references/structural-edge-repair.md#opaque-material-loss-beside-shoes-straps-and-narrow-gaps): distinguish material shading from the actual silhouette, repair source RGB as well as alpha when necessary, and validate positive surfaces and negative spaces separately. Never promote one clip’s repair parameters to a whole-library filter.

For reviewed opaque-material boundary recovery, use `scripts/repair_material_boundary.py` with explicit per-frame source-supported contours and protection masks; see [the local boundary tool](references/structural-edge-repair.md#reusable-reviewed-boundary-tool). It preserves untouched pixels and records review as pending, not accepted.
For opaque interior holes already present in an early matte, use `scripts/restore_opaque_material.py` with reviewed per-frame opaque masks and matched source RGB. Its default preserves valid straight RGB/despill and recovers source color only for erased pixels; use source replacement explicitly for confirmed color damage. Follow [coverage and color separation](references/structural-edge-repair.md#restore-coverage-without-undoing-valid-color-work). Preserve real gaps and graded boundaries; mask creation and motion/export acceptance remain separate reviews.
When missing opaque material and dirty contact edges occur together, use [separate material-hole and contact-boundary diagnosis](references/structural-edge-repair.md#separate-material-holes-from-contact-boundary-residue). Alpha addition and background removal need separate reviewed scopes; a successful local repair is not a whole-library acceptance.

Preserve plausible source motion blur: fast movement need not be sharp in every frame. Judge direction, extent and continuity against neighboring frames and normal-speed playback; follow [motion-blur preservation](references/structural-edge-repair.md#preserve-plausible-motion-blur) when repairing soft moving edges. Clean output is not uniformly sharpened output.
...
For library-wide repairs, follow the [library acceptance and delivery gate](references/structural-edge-repair.md#library-acceptance-and-delivery-gate): verify stable placement, plausible motion and actual playback within clips, at loop boundaries and across reachable transitions. Unrequested flicker or position/scale jumps fail acceptance. If the user conditions commit/push on completion, finish that gate before submitting the requested changes; do not substitute a tooling-only commit for finished assets.
...
Users may select any combination of **GIF**, **APNG**, **RGBA PNG frames**, **Sprite Sheet**, **MOV**, **WebM**, **MP4**, or **complete asset pack** (frames, sheets, metadata and previews). Treat these as output choices, independent of Auto / Preserve artwork / AI repair. Honor explicit selections; do not require a format questionnaire for every task. GIF is the default output when no format is selected. Retain RGBA masters; optionally include APNG for soft-alpha sharing. For explicit game use prioritize PNG frames plus sheets/metadata. A long-video request should use a video container, with GIF only as an optional short preview. See [references/video-export.md](references/video-export.md) for MOV/WebM export. Ask about a target engine only when an engine-specific importer/material is required; generic sprites can be prepared immediately.
...
Users may choose **reference pack only** or **continue generation**. Without authorization for another generation, prepare the reference pack and stop. Existing generation authorization remains valid within its stated scope; do not ask again unnecessarily. A request to update the skill or documentation does not itself authorize a production run. Codex coordinates the chain; automatic keyframe selection and a chain runner are not bundled. The fal client supports multi-reference generation, but not localized video editing, Director streaming or automatic extension. Check the actual provider route before dispatch; do not silently substitute an image-to-video call for requested video editing or extension.
