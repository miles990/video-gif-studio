...
Plan the full spatial and temporal envelope of the character, weapon sweep, travel path, detached trails, particle expansion and final dissipation. Unless the user explicitly requests cropping or off-screen effects, keep all of these inside the canvas with adequate margin throughout the shot. Choose margins from the action rather than applying one universal percentage. Start/end poses alone do not bound the effect: particles may reach their greatest extent after the body stops.

Inspect source transitions and decoded output at maximum release, travel extremes and late decay. Check all four edges for truncated weapon tips, clipped arcs, trails or particles, and verify that the export retains the decay frames. A foreground touching an edge is a review cue, not conclusive proof of clipping; a passing alpha-encoding check cannot prove completeness. Frame the generation wider or pad the reference before a justified retry when necessary. Adding empty border after the source has already clipped an effect does not restore missing pixels. Use one shared canvas and consistent scale; do not fit, zoom or recenter each frame separately to hide framing failures. Keep natural character travel intact.
