...
The bundled exporter currently writes selected RGBA PNG frames, GIF, optional APNG and a provenance/timing manifest. It does NOT bundle an atlas packer, engine importer, independent VFX-layer extraction, or a temporal-matting model. Atlas packing, engine resources and game-specific metadata require additional implementation and verification for the task; describe these as planned until delivered. Animated WebP is also not a bundled output option.
