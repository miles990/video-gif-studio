...
**為什麼使用 Grok：**讓 Grok 一起生成連續動作與特效，提供換腳、重心轉移、攻擊等動作的中間過程，減少獨立姿勢圖直接切換的跳動感；repo 也已整合影片提交、查詢與下載流程。再由 Codex 處理來源畫格，減少反覆重繪造成的角色漂移，檢查結果並輸出。Grok 不保證解剖或物理完全正確，仍須驗收；沒有 Grok 也能使用圖片關鍵幀替代。RGBA 主檔保留柔邊透明度，GIF 則方便分享。
