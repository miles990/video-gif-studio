...
支援有／無參考圖的角色 GIF 製作。**預設使用 Grok 生成連續動作與特效**，以改善動作自然度與連續性，尤其適合複雜動作。**沒有 Grok 也可以生成 GIF**：Grok 不可用或使用者選擇不使用時，由 Codex 使用可用圖片工具製作關鍵幀，或使用既有素材。Codex 負責生成協調、時長、去背與輸出；圖片工具可使用環境提供的模型，**GPT-Image-2.5 可用時亦可使用**。實際品質仍須檢查。
...
- **Grok（預設生成路徑，非必要依賴）**：只有 Grok 影片路徑需要登入、影片權限與額度；repo 已內建影片提交、查詢與下載功能，OAuth 使用官方 Grok CLI 登入，也可明確選用不需 CLI 的 API key 模式，詳見 [Grok 使用方式](references/grok.md)。
...
製作流程：**參考圖或原創角色 → 預設以 Grok 生成影片 → 視需要去背 → PNG 畫格、選用 APNG 與 GIF**。Grok 不可用或明確選擇不使用時，也能從關鍵幀產生新的 GIF，不只限於既有素材轉檔。既有影片可從去背階段開始。未指定時，動作遵循合理人體結構、重心轉移與位移；人物、武器和特效至消散都須完整留在畫面內。要求無縫循環時須檢查實際接點。

**Grok 可用時，特效預設交由 Grok 處理。** 需要特效但未特別指定設計或生成來源時，由 Grok 在動作影片中自行設計並生成特效。Codex 負責規劃特效時機、物理因果、動作可讀性與完整構圖，再進行去背及輸出；使用者有明確指定時，則依指定內容製作。
...
新動作預設由 Grok 生成，除非使用者指定其他路徑；既有素材轉檔不需要重新生成。當服務不可用或額度用盡，可使用現有影片、可用圖片工具生成一致的關鍵幀，或讓合適的既有圖層做有限動畫。眨眼、表情等簡單動作仍可製作 GIF；複雜肢體動作可能仍需要可用的影片服務。成品會標示實際來源與限制，不自動重送失敗請求或切換付費服務／帳號。詳見 [替代流程](references/fallback.md)。
